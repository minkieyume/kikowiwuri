...
(define-module (yumieko builder tags)
...
(define* (tags->page #:key (theme yumieko-theme))
  (lambda (site posts)
    (flat-map (match-lambda
                ((tag . posts)
                 (serialized-artifact (tag-uri tag)
				      (with-layout theme site "Tags" (tags-template site posts #:title tag))
				      sxml->html)))
              (group-by-tag posts))))

(define* (tag-page #:key (theme yumieko-theme))
  (lambda (site posts)
    (serialized-artifact "tags/index.html"
			 (with-layout
			  theme
			  site
			  "Tags"
